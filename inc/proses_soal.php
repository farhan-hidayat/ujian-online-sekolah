...
if (!empty($pilganda) and !empty($esay)) {
  ...
  if (!empty($_POST['soal_pilgan'])) {
    ...
    $persen = $benar / $jumlah;
    $hasil = $persen * 100;
    mysqli_query($db, "INSERT INTO tb_nilai_pilgan VALUES (null, '$id_tq', '$_SESSION[siswa]', '$benar', '$salah', '$tidakjawab', '$hasil')") or die('33' . $db->error);
  } else if (empty($_POST['soal_pilganda'])) {
    $jumlah = $_POST['jumlahsoalpilgan'];
    mysqli_query($db, "INSERT INTO tb_nilai_pilgan VALUES (null, '$id_tq', '$_SESSION[siswa]', '0', '0', '$jumlah', '0')") or die('36' . $db->error);
  }
  ...
  if (!empty($_POST['soal_essay'])) {
    foreach ($_POST['soal_essay'] as $key2 => $value) {
      ...
      while ($data = mysqli_fetch_array($cek)) {
        mysqli_query($db, "INSERT INTO tb_jawaban VALUES(null, '$id_tq','$data[id_essay]','$_SESSION[siswa]','$jawaban')") or die('44' . $db->error);
      }
    }
  } else if (empty($_POST['soal_esay'])) {
    mysqli_query($db, "INSERT INTO tb_jawaban VALUES(null, '$id_tq','$data[id_essay]','$_SESSION[siswa]',null)") or die('48' . $db->error);
  }
...
if (empty($pilganda) and !empty($esay)) {
  if (!empty($_POST['soal_essay'])) {
    foreach ($_POST['soal_essay'] as $key2 => $value) {
      ...
      $cek = mysqli_query($db, "SELECT * FROM tb_soal_essay WHERE id_essay = '$key2'");
      while ($data = mysqli_fetch_array($cek)) {
        mysqli_query($db, "INSERT INTO tb_jawaban VALUES(null, '$id_tq', '$data[id_essay]', '$_SESSION[siswa]', '$jawaban')") or die('61' . $db->error);
      }
    }
  } else if (empty($_POST['soal_essay'])) {
    mysqli_query($db, "INSERT INTO tb_jawaban VALUES(null, '$id_tq', '$data[id_essay]', '$_SESSION[siswa]',null)") or die('65' . $db->error);
  }
  echo "<script>window.location='./../?page=quiz&action=infokerjakan&id_tq=" . $id_tq . "';</script>";
}
...
if (!empty($pilganda) and empty($esay)) {
  if (!empty($_POST['soal_pilgan'])) {
    ...
    $persen = $benar / $jumlah;
    $hasil = $persen * 100;
    mysqli_query($db, "INSERT INTO tb_nilai_pilgan VALUES(null, '$id_tq', '$_SESSION[siswa]', '$benar', '$salah', '$tidakjawab', '$hasil')") or die(' 89 ' . $db->error);
  } else if (empty($_POST['soal_pilgan'])) {
    $jumlah = $_POST['jumlahsoalpilgan'];
    mysqli_query($db, "INSERT INTO tb_nilai_pilgan VALUES(null, '$id_tq', '$_SESSION[siswa]', '0', '0', '$jumlah', '0')") or die('93' . $db->error);
  }
  echo "<script>window.location='./../?page=quiz&action=infokerjakan&id_tq=" . $id_tq . "';</script>";
}
